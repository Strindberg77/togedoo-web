...
Steder (kind='place') har i tillegg feltene `surface`, `lit` og `website`
— utvalgte OSM-tagger fra `osm_tags`-kolonnen (migrasjon 0008; alle rå
tagger lagres ved import, men bare felter med reell dekning eksponeres:
tag-proben jul. 2026 viste surface 63,7 % / lit 27,2 % på Ballbane og
website 64,4 % på Museum). `website` valideres til http/https (med
https-prefiks for skjemaløst OSM-slurv) før den serveres, siden appen
åpner den i webview. Feltene fylles fra data som allerede ligger i
`osm_tags`.
...
| Faste steder (lekeplasser, ballbinger, parker, idrettshaller, badeplasser, museer) | Månedlig batch-import fra OpenStreetMap Overpass: `npx tsx scripts/import-places.ts` (dry-run først). Titler fra OSM-navn, ellers revers-geokodet «Lekeplass ved <gate>», så «Badeplass i <bydel>» (Nominatim bydel/nabolag) for steder uten adresse innen 200 m (`lib/places.ts`). Rader med `locked=true` røres aldri. Krever migrasjon 0005. ODbL-KRAV: «© OpenStreetMap contributors» synlig der stedene vises. Museer (tourism=museum) tatt inn jul. 2026 etter tag-probe: 132 steder i Oslo/Bergen/Stavanger, 97 % navn, fee 62,1 %, charge med kronebeløp 12,9 % (→ price_text). Utsatt til senere vurdering (for tynn OSM-dekning nå): svømmehall/badeland, ishall, akebakke, fornøyelsespark, minigolf, klatrepark | Lav; månedlig kjøring |
